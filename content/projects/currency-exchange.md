...
- Подключиться к серверу через SSH, установить Go
...
- Создать заготовку Go бэкенд приложения с std http server
...
- Нечёткие границы между слоями Controller/Service/Repository.
- Контроллеры, отвечающие за слишком большое количество задач. Они должны отвечать за валидацию, сериализацию JSON ответов, обработку ошибок от слоя Service
- Неправильная или неполная обработка ошибок. Предпочтительный вариант - слой Service возвращает ошибки, controller их обрабатывает и формирует ответ с нужным кодом и телом, используя ошибки со слоя service, а не со слоя repository
...
- Неиспользование моделей со слоя controller для формирования ответов REST API
- Использование одной модели reponse для нескольких ручек
