...
- [Testcontainer](https://github.com/testcontainers/testcontainers-go), позволяет запускать необходимые для тест кейсов инструменты (базы данных, очереди) в Docker контейнерах.
...
Правила к тестам:
- Файлы с тестами должны называться `*_test.go`.
-Тестовые функции - `func TestFoo(t *testing.T)`, где Foo начинается с заглавной буквы.
- Для конкурентных тестов используйте `t.Parallel()` - но только если тест изолирован.

Базовые команды:

- `go test` - Запустить все тесты в текущем пакете
go test
- `go test -v` - Запустить с выводом пройденных тестов (-v = verbose)
- `go test ./...` - Запустить тесты во всех подпакетах рекурсивно
- `go test -run Foo` - Запустить конкретный тест (по имени)
- `go test -cover` - Запустить с измерением покрытия (в процентах)
...
- `github.com/stretchr/testify` - упрощает assertions (require, assert) и моки (mock.Mock).
- `golang/mock` - генерация mock-реализаций интерфейсов.
- `go-sqlmock`  - мокинг *sql.DB / *sqlx.DB.
- `go test -cover`, `go tool cover` — измерение покрытия.
     

#### Избранные курсы и учебные ресурсы

- [Генерация и использование моков в Go](https://www.youtube.com/watch?v=qaaa3RsC0FQ)
- [Статья об общих подходах в unit тестах](https://habr.com/ru/articles/575368/)
- [Изучения Go с помощью тестов](https://quii.gitbook.io/learn-go-with-tests)
...
- [Testcontainer](https://github.com/testcontainers/testcontainers-go) - библиотека для запуска экземпляров нужных для тестов инструментов внутри Docker контейнеров
...
- [Видео о TestContainers](https://www.youtube.com/watch?v=sNg0bnMF_qY)
